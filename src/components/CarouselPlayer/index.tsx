  ...
  return (
    <div className="relative">
      <div className="overflow-hidden w-full h-80 md:h-96 xl:h-[488px]">
        <div
          className="flex  h-80 md:h-96 xl:h-[488px]"
          style={{
        ...
        >
          {playlist.map((video) => (
            <div
              key={video._key}
              className="w-full object-cover  flex-shrink-0  h-80 md:h-96 xl:h-[488px]"
            >
              <ReactPlayer
                url={video.url}
        ...
        {playlist.map((_, index) => (
          <button
            ...
            className={`w-4 h-4 rounded-full ${
              index === currentVideo
                ? "bg-gray-50 w-8 transform transition-all duration-500"
