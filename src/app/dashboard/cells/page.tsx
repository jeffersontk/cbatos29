...
export default function CellsPage() {
  ...
  return (
    <DashboardLayout
      title="Celulas e expansao"
      subtitle="A plataforma conecta cadastro de membros, familias, bairro, lideranca e abertura de novas celulas a partir de demanda real."
      actions={[
    ...
    >
      <section className="grid gap-4 md:grid-cols-3">
        <KpiCard title="Celulas ativas" value={String(cellCoverage.length)} helper="Distribuidas por bairros e com foco pastoral definido." icon={Home} />
        <KpiCard title="Pessoas em cuidado" value={String(totalPeopleInCare)} helper="Total da celula considerando familiares vinculados." icon={UserRoundPlus} />
        <KpiCard title="Vagas disponiveis" value={String(totalVacancies)} helper="Base para alocar novos interessados por proximidade." icon={Map} />
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_1fr]">
        ...
        <Card className="border-border/70">
          <CardHeader>
            <CardTitle className="text-xl">Pedidos para novas celulas</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {cellRequests.map((request) => (
              <div key={request.id} className="rounded-2xl border border-border/70 bg-background p-4">
                ...
                <p className="mt-2 text-xs text-muted-foreground">Lideranca potencial: {request.potentialLeader}</p>
              </div>
            ))}
          </CardContent>
        </Card>
